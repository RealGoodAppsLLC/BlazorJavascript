...
namespace RealGoodApps.BlazorJavascript.CodeGenerator
{
    public class CodeGenerator
    {
        ...
        private static string RenderParameterArray(ProcessedParametersInfo parameters)
        {
            var parameterNamesRendered = new StringBuilder();

            parameterNamesRendered.Append("var parametersAsArray = new List<IJSObject>();");

            foreach (var parameter in parameters.Items)
            {
                parameterNamesRendered.Append(Environment.NewLine);

                if (parameter.IsDotDotDot)
                {
                    parameterNamesRendered.Append($"parametersAsArray.AddRange({parameter.Name});");
                }
                else
                {
                    parameterNamesRendered.Append($"parametersAsArray.Add({parameter.Name});");
                }
            }
            ...
            string? implementationPrefix)
        {
            ...
            else
            {
                ...
                var renderedParametersArray = RenderParameterArray(parameters);
                stringBuilder.AppendLine(RenderIndented(renderedParametersArray, 1));

                if (isConstructor)
                {
                    stringBuilder.AppendLine(RenderIndent(1) + $"return this.CallConstructor<{returnTypeName}>(parametersAsArray.ToArray());");
                }
                else
                {
                    stringBuilder.AppendLine(RenderIndent(1) + $"var methodObj = this.GetPropertyOfObject<IFunction>(\"{internalMethodName}\");");

                    if (returnType == null)
                    {
                        stringBuilder.AppendLine(RenderIndent(1) + $"methodObj.InvokeVoid(this, parametersAsArray.ToArray());");
                    }
                    else
                    {
                        stringBuilder.AppendLine(RenderIndent(1) + $"return methodObj.Invoke<{returnTypeName}>(this, parametersAsArray.ToArray());");
                    }
                }
